...
namespace Phoenix.Data.Plc.Test
{
	...
	public class ConversionTest
	{
		...
		public void ConvertText()
		{
			...
			var asciiItem = new TextPlcItem(dataBlock: 0, position: 0, initialValue: target, encoding: encoding);
			Assert.True(targetBytes.SequenceEqual((byte[])((IPlcItem)asciiItem).Value));


			// Check data to value.
			asciiItem = new TextPlcItem(dataBlock: 0, position: 0, length: (ushort) targetBytes.Length, encoding);
			...
			// Check overflow.
			asciiItem = new TextPlcItem(dataBlock: 0, position: 0, length: 1, encoding);
